program main
  ...
  character (len=*),parameter :: description = "check"
  ...
  do name = 1, 1, 1    
    clm_data_file='/home/ecaas/nird/all_sites_decomp/'//trim(site_names(name))//'_hist_for_decomp/lnd/hist/'//trim(site_names(name))//'_hist_for_decomp.clm2.'
    ...
    call read_nlayers(trim(adjustr(clm_data_file)//'all.1901.nc'), levels)
    ...
    !2: SPINUP
    call decomp(nsteps=1000*24*365, run_name=trim(trim(site_names(name))//"_"//description//"_"//"Spunup"),nlevdecomp=levels, step_frac=1, write_hour=24*365*100,&
    ...
    call decomp(nsteps=71*24*365, run_name=trim(trim(site_names(name))//"_"//description//"_"//"to1970"),nlevdecomp=levels, step_frac=1, write_hour=1*24*365, &
