program main
  ...
  character (len=*),parameter :: description = "cycle_year"
  character (len=*),parameter :: clm_data_file = '/home/ecaas/nird/'//site//'_historical/lnd/hist/'//site//'_historical.clm2.'
  ...
  !2: SPINUP
  call decomp(nsteps=200*24*365, run_name=trim(site//"_"//description//"_"//"Spunup"),nlevdecomp=levels, step_frac=1, write_hour=1*24*365*10,&
  pool_C_start=C_matrix_init,pool_N_start=N_matrix_init, pool_C_final=C_matrix_Spunup,pool_N_final=N_matrix_Spunup,&
  start_year=1988,stop_year=1988,clm_input_path=clm_data_file,clm_surf_path=clm_surface_file)
  !|
  !| Use output of last timestep to initialize step 2
  ! 
  ! !3: RUN WITH MONTHLY UPDATES of inputdata:
  ! call decomp(nsteps=70*24*365, run_name=trim(site//"_"//description//"_"//"to1970"),nlevdecomp=levels, step_frac=1, write_hour=1*24*5*365, &
  ...
  pool_C_start=C_matrix_Spunup,pool_N_start=N_matrix_Spunup, pool_C_final=C_matrix_final,pool_N_final=N_matrix_final,&
  start_year=1988,stop_year=1988,clm_input_path=clm_data_file,clm_surf_path=clm_surface_file)
  ! 
  ! 
